...
for s in np.arange(len(allsubjects)):
    ...
    for ses in np.arange(len(sessions)):
        ...
        z = f.readline()
        f.close()
        all_subject_averages_happy_neutral[s,ses] = float(z[:-1])

stat = all_subject_averages_happy_neutral
...
stat = all_subject_averages_fear_neutral
fig = plotPosterStyle(stat,allsubjects)
x,y = nonNan(stat[HC_ind,0],stat[MDD_ind,0])
t,p = scipy.stats.ttest_ind(x,y)
addSingleStat(p/2,0,np.nanmax(stat),0.03)
...
fear_neutral = all_subject_averages_fear_neutral.flatten()
happy_neutral = all_subject_averages_happy_neutral.flatten()
ndays=2
day = np.tile(np.arange(ndays),nsubs)
subject = np.repeat(allsubjects,ndays)
groups = ['HC' if i in HC_ind else 'MDD' for i in np.arange(nsubs)]
groups = np.repeat(groups,ndays)
DATA = {}
DATA['fear_vs_neutral'] = fear_neutral
DATA['happy_vs_neutral'] = happy_neutral
DATA['day'] = day
...
g.map(plt.scatter,'groups','fear_vs_neutral',color=['k'],alpha=0.3)
g.map(sns.pointplot,'groups','fear_vs_neutral',palette=pal,ci=68,alpha=0.5,scale=1.5,errwidth=5)
g.add_legend()
plt.show()

pal = dict(HC='k', MDD='r')
g = sns.FacetGrid(df,col='day',palette=pal)
g.map(plt.scatter,'groups','happy_vs_neutral',color=['k'],alpha=0.3)
g.map(sns.pointplot,'groups','happy_vs_neutral',palette=pal,ci=68,alpha=0.5,scale=1.5,errwidth=5)
g.add_legend()
plt.show()
