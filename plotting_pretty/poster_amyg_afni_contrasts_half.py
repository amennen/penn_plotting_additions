...
for s in np.arange(len(allsubjects)):
    ...
    for ses in np.arange(len(sessions)):
        ...
        for trial in np.arange(ntrials):
            output_text = "{0}/{1}_{2}_task-faces_neutral_{3}_half_amgyavg.txt".format(output_path,bids_id,ses_id,trial)
        ...
        for trial in np.arange(ntrials):
            output_text = "{0}/{1}_{2}_task-faces_fearful_{3}_half_amgyavg.txt".format(output_path,bids_id,ses_id,trial)
            f = open(output_text,"r") 
            z = f.readline()
            f.close()
        ...
        # now get all happy
        for trial in np.arange(ntrials):
            output_text = "{0}/{1}_{2}_task-faces_happy_{3}_half_amgyavg.txt".format(output_path,bids_id,ses_id,trial)
            f = open(output_text,"r") 
            z = f.readline()
            f.close()
...
diff_stat = all_subject_averages_fearful[:,1,:] - all_subject_averages_fearful[:,0,:]
# just plot over all day first
fig = plotPosterStyle_multiplePTS(stat,allsubjects)
plt.subplot(1,2,1)
plt.ylim([-.55,.8])
...
# NOW HAPPY FACES
stat = all_subject_averages_neutral
diff_stat = all_subject_averages_neutral[:,1,:] - all_subject_averages_neutral[:,0,:]
# just plot over all day first
fig = plotPosterStyle_multiplePTS(stat,allsubjects)
plt.subplot(1,2,1)
plt.ylim([-.55,.7])
...
diff_stat = all_subject_averages_happy[:,1,:] - all_subject_averages_happy[:,0,:]
# just plot over all day first
fig = plotPosterStyle_multiplePTS(stat,allsubjects)
plt.subplot(1,2,1)
plt.ylim([-.55,.9])
...
t,p = scipy.stats.ttest_ind(x,y)
addComparisonStat_SYM(p/2,0,1,np.nanmax(stat),0.05,0,' ')
x,y = nonNan(stat[MDD_ind,0,1],stat[HC_ind,0,1])
t,p = scipy.stats.ttest_ind(x,y)
addComparisonStat_SYM(p/2,-.1,.1,np.nanmax(stat)+.2,0.05,0,'MDD < HC ')
x,y = nonNan(stat[MDD_ind,1,1],stat[HC_ind,1,1])
t,p = scipy.stats.ttest_ind(x,y)
addComparisonStat_SYM(p/2,.9,1.1,np.nanmax(stat)+.2,0.05,0,'MDD < HC ')
labels_pos_v = np.array([-.5,0,0.5])
labels_pos = labels_pos_v.astype(np.str)
...
x,y = nonNan(stat[HC_ind,0,0],stat[MDD_ind,0,0])
t,p = scipy.stats.ttest_ind(x,y)
print('half 0 1-sided p=%2.2f' % (p/2))
x,y = nonNan(stat[HC_ind,1,0],stat[MDD_ind,1,0])
t,p = scipy.stats.ttest_ind(x,y)
print('half 1 1-sided p=%2.2f' % (p/2))
...
stat = all_subject_averages_fearful[:,1,:] - all_subject_averages_fearful[:,0,:]
#stat = total
...
stat = np.mean(all_subject_averages_fearful[:,:,:],axis=1)
#stat = total
fig,ax = plotPosterStyle_DF(stat,allsubjects)
plt.xticks(np.arange(2),('Pre NF', 'Post NF'))
plt.ylabel(r'LA negative face $\beta$')
x,y = nonNan(stat[HC_ind,0],stat[MDD_ind,0])
t,p = scipy.stats.ttest_ind(x,y)
addComparisonStat_SYM(p/2,-.2,.2,np.nanmax(stat),0.05,'$MDD_1 > HC_1$')
x,y = nonNan(stat[HC_ind,1],stat[MDD_ind,1])
...
stat = all_subject_averages_fearful[:,1,:] - all_subject_averages_neutral[:,1,:]
#stat = total
fig,ax = plotPosterStyle_DF(stat,allsubjects)
plt.xticks(np.arange(2),('Pre NF', 'Post NF'))
...
stat = np.mean(all_subject_averages_fearful[:,:,:],axis=1) - np.mean(all_subject_averages_neutral[:,:,:],axis=1)
#stat = total
fig,ax = plotPosterStyle_DF(stat,allsubjects)
plt.xticks(np.arange(2),('Pre NF', 'Post NF'))
plt.ylabel('average beta')
...
stat = all_subject_averages_neutral[:,1,:] - all_subject_averages_neutral[:,0,:]
#stat = total
fig,ax = plotPosterStyle_DF(stat,allsubjects)
plt.xticks(np.arange(2),('Pre NF', 'Post NF'))
plt.ylabel('average beta')
...
# INIITALLY - are depression symptoms related to first response to neg or last response??
M = getMADRSscoresALL()
colors = ['k', 'r'] # HC, MDD
fig = plt.figure(figsize=(10,7))
for s in np.arange(nsubs):
  subjectNum  = allsubjects[s]
  madrs_1 = M[subjectNum][0]
  if subjectNum < 100:
    style = 0
  elif subjectNum > 100:
    style = 1
  plt.plot(all_subject_averages_fearful[s,1,0],d2[s],marker='.',ms=20,color=colors[style],alpha=0.5)
plt.xlabel('Amyg response late in trial')
plt.ylabel('V1 MADRS')
#plt.xlim([-0.4,0.4])
plt.show()
