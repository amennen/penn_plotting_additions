...
nreps=3
nhalves=2
nperstim=nreps*nhalves
all_subject_averages_neutral = np.zeros((nsubs,nperstim,2))
all_subject_averages_fearful = np.zeros((nsubs,nperstim,2))
all_subject_averages_happy = np.zeros((nsubs,nperstim,2))
    ...
    for ses in np.arange(len(sessions)):
        ...
        for r in np.arange(nreps):
          for h in np.arange(nhalves):
            output_text = "{0}/{1}_{2}_task-faces_neutral_{3}_{4}_over_time_amgyavg.txt".format(output_path,bids_id,ses_id,r,h)
        ...
        for r in np.arange(nreps):
          for h in np.arange(nhalves):
            output_text = "{0}/{1}_{2}_task-faces_fearful_{3}_{4}_over_time_amgyavg.txt".format(output_path,bids_id,ses_id,r,h)
            f = open(output_text,"r") 
            z = f.readline()
            f.close()
        ...
        for r in np.arange(nreps):
          for h in np.arange(nhalves):
            output_text = "{0}/{1}_{2}_task-faces_happy_{3}_{4}_over_time_amgyavg.txt".format(output_path,bids_id,ses_id,r,h)
            f = open(output_text,"r") 
            z = f.readline()
            f.close()
