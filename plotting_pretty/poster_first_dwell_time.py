...
fixation_durations_first_time =np.zeros((nsubjects,ntrials,ndays,nemotions)) *np.nan
n_fixations_per_trial = np.zeros((nsubjects,ntrials,ndays,nemotions)) *np.nan
    ...
    for t in np.arange(ntrials):
      ...
      if len(fixations): # looked somewhere
        ...
        for f in np.arange(nfixations):
          ...
          emotion = c_trial[f][0] - 1
          n_fixations_per_trial[s,t,d,emotion] += 1
          ...
          if n_fixations_per_trial[s,t,d,emotion] == 1:
            ...
            if fixation_durations_first_time[s,t,d,emotion] > 1:
              print(s,d,t,f)
...
trial_average_time = np.nanmean(fixation_durations_first_time,axis=1)
all_data=trial_average_time
fig,ax=plotPosterStyle_DF_valence(all_data[:,np.array([0,2,3]),:],subjects,emo,'ratio of time, first viewing')
# plt.subplot(1,2,1)
...
n_fix_avg = np.nanmean(n_fixations_per_trial,axis=1)
# want to divide for each trial by the sum of all fixations for that trial
#n_fix_per_trial_all = n_fixations_per_trial/np.sum(n_fixations_per_trial,axis=3) # this isn't used again so deleting
stat = trial_average_time[:,:,DYSPHORIC-1]
topL=0.4
...
#n_fix_per_trial_all = n_fixations_per_trial/np.sum(n_fixations_per_trial,axis=3) # this isn't used again so deleting
stat = trial_average_time[:,:,POSITIVE-1]
topL=0.4
fig,ax = plotPosterStyle_DF(stat[:,np.array([0,1,2,3])],subjects)
plt.ylim([0,.8])
plt.xticks(np.arange(4),('Pre NF', 'Mid NF' ,'Post NF', '1M FU'))
...
emo_diff = trial_average_time[:,:,POSITIVE-1] - trial_average_time[:,:,DYSPHORIC-1]
stat = emo_diff
topL=0.4
fig,ax = plotPosterStyle_DF(stat[:,np.array([0,1,2,3])],subjects)
#plt.ylim([0,.8])
...
improvement_first = stat[:,2] - stat[:,0]
improvement = emo_diff[:,2] - emo_diff[:,0]
M = getMADRSscoresALL()
...
for s in np.arange(nsubjects):
  ...
  if subjectNum < 100:
    ...
    style = 1
  #plt.plot(-1*improvement_first[s],-1*d1[s],marker='.',ms=30,color=colors[style])
  #plt.plot(stat[s,0],M[subjectNum][0],marker='.',ms=30,color=colors[style])
  plt.plot(improvement[s],M[subjectNum][1],marker='.',ms=30,color=colors[style])
...
x,y = nonNan(improvement[MDD_ind],-1*d1[MDD_ind])
...
for s in np.arange(nsubs):
  ...
  if subjectNum < 100:
    ...
    style = 1
  plt.plot(-1*all_neg_change[s],-1*improvement_first[s],marker='.',ms=30,color=colors[style])
...
emo_diff = trial_average_total_viewing[:,:,DYSPHORIC-1] -trial_average_total_viewing[:,:,POSITIVE-1]
...
plt.ylabel('mean ratio total time - dysphoric - positive')
plt.show()
