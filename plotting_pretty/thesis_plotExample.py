...
def getOpacityFromFile(subjectNum,day,run,block):
  ...
  # file = glob.glob(os.path.join(runDir,'patternsdata_*.mat'))[0]
  # dp = utils.loadMatFile(file)
  # reg = dp.patterns.regressor
...
run = 5 # actually run 7 for matlab/what the subject experienced
day = 0
blockNum=3 # this ipython index 3 -- so it's really the 4th block of neurofeedback
nTRpBlock=25
...
plt.ylabel('scene - face',fontsize=40)
...
plt.savefig('poster_plots/clf.png')
#plt.show()
...
#plt.title('p(b|a)')
#plt.colorbar()
plt.savefig('thesis_plots_checked//ex_matrix_nocolorbar.png')
plt.show()
...
labels_pos = labels_pos_v.astype(np.str)
plt.yticks(np.array([0,16]),labels_pos,fontsize=30)
plt.xticks(fontsize=30)
plt.savefig('thesis_plots_checked//binned_clf.png')
plt.show()


# make example amygdala response
